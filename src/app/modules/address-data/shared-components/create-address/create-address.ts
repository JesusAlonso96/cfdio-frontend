...
import {
  ...
  HOUSE_NUMBER_REGEX,
  ZIP_CODE_REGEX,
} from '../../../../shared/regular-expresions/address.regex';
...
export class CreateAddressComponent extends BaseFormComponent<CreateAddress> implements OnInit {
  ...
  private readonly _externalCatalogsService = inject(ExternalCatalogsService);
  ...
  private getZipCodeData(): void {
    this.form
      ...
      .subscribe(async (cp) => {
        ...
        if (new RegExp(ZIP_CODE_REGEX).test(cp)) {
          this.loadingZipCodeData.set(true);
          const response: ZipCodesResponse =
            await this._externalCatalogsService.getZipCodeDataAsync(cp);
          this.setZipCodeData(
            response.state,
            response.municipality,
            response.neighborhoods.length === 1 ? response.neighborhoods[0] : '',
          );
  ...
  async onSubmit() {
    try {
      ...
      const addressDataCreated: AddressData =
        await this._addressDataService.createAddressDataAsync(addressData);
      ...
      this._toastService.showError(
        'Ocurrió un error al crear la dirección, por favor intentalo de nuevo más tarde',
      );
    }
  }
}
